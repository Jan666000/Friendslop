...
namespace Friendslop 
{
    public class LobbyUIManager : NetworkBehaviour
    {
        ...
        public void UpdatePlayerLobbyUI() 
        {
            ...
            List<CSteamID> orderdMembers = new List<CSteamID>();
            
            if(memberCount == 0) 
            {
                Debug.LogWarning("Lobby has no players. Retrying...");
                StartCoroutine(RetryUpdate());
                return;
            }

            orderdMembers.Add(hostID);
            ...
            for(int i = 0; i < memberCount; i++) 
            {
                ...
                if(memberID != hostID) 
                {
                    orderdMembers.Add(memberID);
                }
            }

            int j = 0;
            foreach(var member in orderdMembers) 
            {
